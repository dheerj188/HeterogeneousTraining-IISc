...
class CPUTrainer:
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            times_per_epoch.append((ee_time-es_time)/3600)
...
def main(rank):
    ...
    print(f"CPU execution with Intel Xeon Gold Processor with rank {rank}, without ipex")

    torch.set_num_threads(2**rank)

    print(f"executing with {2**rank} threads")
    ...
    print(f"Total execution time for {epochs} epochs and with {2**rank} threads is: {sum(times)} hours")
...
if __name__ == "__main__":
    ...
    for rank in range(world_size):
        ...
        process = mp.Process(target = main, args = (rank,))
