...
from commons.model_zoo_mod import model_builder
...
from commons.data_load_k import initialize_data_loader
...
def main():
    ...
    if args.distplan.startswith("CAI"):
        ...
        if args.distplan.startswith("CAI_ZeRO"):
            ...
            plugin = GeminiPlugin(search_range_m=128, 
        ...
        """Dataset loading and processing (GLUE)"""
        model_name = "gpt2"

        # data_builder = GLUEDataBuilder(
        #     model_name, plugin, train_batch_size=BATCH_SIZE, eval_batch_size=BATCH_SIZE
        # )

        # train_dataloader = data_builder.train_dataloader()
        # test_dataloader = data_builder.test_dataloader()

        train_dataloader, _ = initialize_data_loader(BATCH_SIZE, 0)
    ...
    with torch.profiler.profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./log9b/gemgptx1'),
        record_shapes=True,
        #profile_memory=True,
        with_stack=True
        ) as prof:
        ...
        """
        for n, batch in enumerate(train_dataloader_iter):
            batch = next(train_dataloader_iter)
            train_step(batch)
            prof.step()
            
    prof.export_chrome_trace(f"./logchrome9b/cai_gem_ha_trace-rank{profile_rank}.json")
    tflops_list.sort()
